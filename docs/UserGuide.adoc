...
Persistent, meaning the in subsequent `list` and `find` calls, persons are still sorted.
Does not garuntee address book is sorted after adding additional persons after `sort`.
****
...
* `add John Doe p/98765432 e/johnd@gmail.com a/John street, block 123, #01-01` +
 `add Betsy Crowe pp/1234567 e/betsycrowe@gmail.com pa/Newgate Prison t/criminal t/friend` +
 `sort` +
Sorts the address book, such that Betsy is listed before John in subsequent `list` or `find` calls.
